...
def update_userline(user_id, isComment, timestamp, comment_id):
    db["users"].update_one(user_id, {"$push":{"userline": {"interactionType": int(isComment), "timestamp": timestamp, "post": comment_id}}})
...
def update_topic(topic_id_dict,user_id_dict,post_id):
    ...
    if (resp_len == 0):
        add_ret = db["topics"].insert_one({"_id": topic_id_dict["_id"], "posts": [], "postCount": 0,"usersFollowing":[],"followersCount":0})
...
def create_post(data: dict) -> bool:
    ...
    return_val = db["posts"].insert_one(
        {"topic": topic, "user": user, "timestamp": timestamp, "likeCount": likeCount, "dislikeCount": dislikeCount,
         "isComment": isComment, "contentType": content_type, "content": content, "comments": comments, "commentCount": commentCount,
         "parentID": parent_id})
