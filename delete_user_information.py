from helpers import check_for_data, db, generate_confirmation_code, send_email, ip
...
def delete_user_without_conf_code(data: dict, live: bool = True) -> bool:
    ...
    if live:
        ...
        if status.modified_count != 1:
            return False
        send_email(to_email=email, text=f"{ip}:5000/delete_user?confirmation_code={confirmation_code_generated}&email={email}", subject="Confirmation Code")
        return True